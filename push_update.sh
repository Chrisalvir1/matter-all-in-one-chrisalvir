...
git commit -m "fix(matter): preserve paired legacy endpoints when composite restore fails" || echo "Nada que comitear"
echo "Creando tag..."
git tag -a v1.2.46 -m "v1.2.46 — preserve paired legacy endpoints when composite restore fails" || echo "El tag ya existe"
echo "Haciendo push..."
