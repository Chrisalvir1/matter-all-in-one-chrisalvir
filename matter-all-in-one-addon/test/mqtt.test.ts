...
import { MqttClientManager, MqttDiscoveryEntry } from '../src/mqtt/mqtt-client.js';
import { onOffLight, dimmableLight, onOffPlugInUnit, temperatureSensor } from 'matterbridge';
...
describe('MQTT Auto-Discovery and Entity Mapping', () => {
  it('correctly determines Matter device types based on component and device class', () => {
    ...
    expect(getMqttDeviceType('light', { brightness: true })).toEqual(dimmableLight);
    expect(getMqttDeviceType('switch', {})).toEqual(onOffPlugInUnit);
    expect(getMqttDeviceType('sensor', { device_class: 'temperature' })).toEqual(temperatureSensor);
  });

  it('creates MqttEntity with clean identifiers and metadata', () => {
    const mockLog: any = { info: vi.fn(), debug: vi.fn(), warn: vi.fn(), error: vi.fn(), notice: vi.fn() };
    const mockPlatform: any = { log: mockLog, isDpsGenericEntity: () => false };
    const mockMqttManager = new MqttClientManager(mockLog, { host: '127.0.0.1' });
