...
describe("CompositeDeviceEntity", () => {
  ...
  it("sends fan and light commands to their own HA services", async () => {
    ...
    await (composite.endpoints.get("fan.sala") as any).invokeCommand(
      "OnOff.on",
    );
    await (composite.endpoints.get("light.sala") as any).invokeCommand(
      "OnOff.on",
    );
      ...
      [
        {
          ...
          state: state("humidifier.difusor_sala", "on", {
            min_humidity: 40,
            max_humidity: 80,
            humidity: 60,
          }),
