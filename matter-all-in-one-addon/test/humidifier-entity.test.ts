...
const platform = {
  ...
  ha: {
    ...
    hassEntities: new Map([['humidifier.aroma_diffuser', { id: 'entity-diffuser', device_id: 'device-diffuser' }]]),
    hassDevices: new Map([['device-diffuser', { serial_number: 'DIFFUSER-REAL-SN' }]]),
  },
};
...
describe('HumidifierEntity diffuser vapor & mode controls', () => {
  it('maps slider 10% to set_humidity on humidity-based diffusers', async () => {
    const entity = new HumidifierEntity(platform as any, diffuserState({ available_modes: [] }), MatterDeviceTypes.fan);
    ...
    expect(platform.ha.callService).toHaveBeenLastCalledWith('humidifier', 'set_humidity', 'humidifier.aroma_diffuser', {
      humidity: 10,
    });
    ...
    await new Promise((r) => setTimeout(r, 60));
    expect(platform.ha.callService).toHaveBeenLastCalledWith('humidifier', 'turn_off', 'humidifier.aroma_diffuser');
  });

  it('maps slider to mode for mode-based diffusers without humidity range', async () => {
    const entity = new HumidifierEntity(platform as any, diffuserState({
      humidity: undefined,
    ...
    await endpoint.invokeAttributeChange(514, 'percentSetting', 10);
    await new Promise((r) => setTimeout(r, 60));
    expect(platform.ha.callService).toHaveBeenLastCalledWith('humidifier', 'set_mode', 'humidifier.aroma_diffuser', {
      mode: 'low',
    });
  ...
  it('handles fanMode writes (Auto vs Manual) via set_mode', async () => {
    const entity = new HumidifierEntity(platform as any, diffuserState(), MatterDeviceTypes.fan);
    const endpoint = await entity.createEndpoint() as any;
    await entity.syncInitialState();
