...
describe("core converters", () => {
  ...
  it("preserves fan percentages", () => {
    ...
    expect(fanConverter.toHaPercentage(67)).toBeCloseTo(66.67, 1);
  });
  ...
  it("maps lock states and commands safely", () => {
    ...
    expect(lockConverter.toLockState(state("unlocked"))).toBe(
      MatterLockState.Unlocked,
    );
    expect(lockConverter.toLockState(state("jammed"))).toBe(
      MatterLockState.NotFullyLocked,
    );
  ...
  it("rejects non-numeric tariff and soil readings", () => {
    ...
    expect(
      soilSensorConverter.toMoistureValue(state("unavailable")),
    ).toBeNull();
  });
});
