...
describe("haVacuumStateToMatter", () => {
  it("maps cleaning → Running", () => {
    expect(haVacuumStateToMatter("cleaning")).toBe(
      RvcOperationalStateId.Running,
    );
  });
  ...
  it("maps returning → SeekingCharger", () => {
    expect(haVacuumStateToMatter("returning")).toBe(
      RvcOperationalStateId.SeekingCharger,
    );
  });
  ...
  it("falls back unknown state → Stopped", () => {
    expect(haVacuumStateToMatter("some_tuya_weirdness")).toBe(
      RvcOperationalStateId.Stopped,
    );
  });
...
describe("haVacuumIsActive", () => {
  it("returns true when cleaning", () =>
    expect(haVacuumIsActive("cleaning")).toBe(true));
  it("returns false when docked", () =>
    expect(haVacuumIsActive("docked")).toBe(false));
  it("returns false when idle", () =>
    expect(haVacuumIsActive("idle")).toBe(false));
});
...
describe("matterCommandToHaVacuum", () => {
  it("maps start → vacuum.start", () => {
    expect(matterCommandToHaVacuum("start")).toEqual({
  ...
  it("maps pause → vacuum.pause", () => {
    expect(matterCommandToHaVacuum("pause")).toEqual({
      service: "vacuum.pause",
    });
  });

  it("maps resume → vacuum.start (HA has no resume)", () => {
    expect(matterCommandToHaVacuum("resume")).toEqual({
      service: "vacuum.start",
    });
  });

  it("maps goHome → vacuum.return_to_base", () => {
    expect(matterCommandToHaVacuum("goHome")).toEqual({
...
describe("buildVacuumUpdate", () => {
  it("cleaning entity produces Running + onOff=true", () => {
    const entity = makeEntity("cleaning", {
      battery_level: 90,
      fan_speed: "strong",
    });
...
describe("isVacuumEntity", () => {
  ...
  it("matches switch domain when effectiveProfile is roboticVacuumCleaner", () => {
    const switchRobot = {
      ...makeEntity("on"),
      entity_id: "switch.omni_broadlink_robot_limpiador",
    };
...
describe("getSupportedVacuumCleanModes", () => {
  it("filters out standby, chargego, manual and maps valid Robotina clean modes", () => {
    ...
    expect(modes.map((m) => m.option)).toEqual([
      "random",
      "smart",
      "wall_follow",
      "spiral",
