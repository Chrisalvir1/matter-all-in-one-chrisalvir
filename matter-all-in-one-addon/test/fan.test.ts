...
describe("Fan converter — direction (test 7 + test 10)", () => {
  ...
  it('10. matterDirectionToHa(Forward) → "forward"', () => {
    expect(matterDirectionToHa(FanControl.AirflowDirection.Forward)).toBe(
      "forward",
    );
  });

  it('10. matterDirectionToHa(Reverse) → "reverse"', () => {
    expect(matterDirectionToHa(FanControl.AirflowDirection.Reverse)).toBe(
      "reverse",
    );
  });
  ...
  it("haDirectionToMatter(undefined) → Forward por defecto", () => {
    expect(haDirectionToMatter(undefined)).toBe(
      FanControl.AirflowDirection.Forward,
    );
  });
});
...
describe("Fan converter — Feature conformance & capability detection", () => {
  it("detects direction capability strictly via FanEntityFeature.DIRECTION (4)", () => {
    ...
    const livingRoomState = makeState("on", {
    ...
    const bedroomState = makeState("on", {
  ...
  it("selects conformant FanModeSequence dynamically", () => {
    const livingRoomState = makeState("on", { supported_features: 53 });
    expect(getFanModeSequence(livingRoomState)).toBe(
    ...
    expect(getFanModeSequence(bedroomState)).toBe(
      FanControl.FanModeSequence.OffLowMedHighAuto,
    );
    ...
    expect(getFanModeSequence(autoState)).toBe(
      FanControl.FanModeSequence.OffLowMedHighAuto,
    );
  });
