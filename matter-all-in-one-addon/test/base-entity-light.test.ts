...
describe("Light Entity Comprehensive Audit (18 Requirements)", () => {
    ...
    platform = {
      ...
      ha: {
        ...
        hassEntities: new Map([
          [
            "light.govee_rgb",
            { id: "entity-govee", device_id: "device-govee" },
          ],
        ]),
        hassDevices: new Map([
  ...
  async function createLightEntity(
    state: any,
    deviceType = MatterDeviceTypes.colorTemperatureLight,
  ) {
  ...
  it("1. OFF + cached brightness from HA remains OFF in Matter (no ghost power-on)", async () => {
    ...
    expect(endpoint.attributes.get(`${LevelControl.id}:currentLevel`)).toBe(
      lightConverter.toLevel(220),
    );
  });
  ...
  it("2. OFF + cached Kelvin from HA remains OFF in Matter", async () => {
    ...
    expect(endpoint.attributes.get(`${OnOff.id}:onOff`)).toBe(true);
    expect(
      endpoint.attributes.get(`${ColorControl.id}:colorTemperatureMireds`),
    ).toBe(250);
  });
  ...
  it("11, 12, 13. WebSocket reconnect (isInitialSync) recovers state, brightness, and Kelvin regardless of lockout", async () => {
    ...
    expect(endpoint.attributes.get(`${OnOff.id}:onOff`)).toBe(true);
    expect(endpoint.attributes.get(`${LevelControl.id}:currentLevel`)).toBe(
      lightConverter.toLevel(180),
    );
    expect(
      endpoint.attributes.get(`${ColorControl.id}:colorTemperatureMireds`),
    ).toBe(lightColor.kelvinToMireds(4200));
  });
  ...
  it("14. Restart syncInitialState restores complete state from HA", async () => {
    ...
    expect(ep.attributes.get(`${LevelControl.id}:currentLevel`)).toBe(
      lightConverter.toLevel(210),
    );
    expect(ep.attributes.get(`${ColorControl.id}:colorTemperatureMireds`)).toBe(
      lightColor.kelvinToMireds(3500),
    );
  });
  ...
  it("17. ExtendedColorLight continues to map to code 0x010D for RGB/HS/XY", () => {
    const devTypeHs = getLightDeviceType({
  ...
  it("18. Existing Govee RGB/WW lights maintain OnOff, brightness, HS and Kelvin commands without regression", async () => {
    ...
    // Test HS command
    await ep.invokeCommand("moveToHueAndSaturation", {
      hue: 127,
      saturation: 254,
    });
    expect(platform.ha.callService).toHaveBeenCalledWith(
    ...
    await ep.invokeCommand("moveToColorTemperature", {
