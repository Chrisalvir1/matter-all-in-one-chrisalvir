...
class Accessory extends events_1.EventEmitter {
    ...
    configureController(controllerConstructor) {
        ...
        if ((0, controller_1.isSnapshotController)(controller)) { // save the controller for snapshot handling
    ...
    findPermsViolation() {
        for (const service of this.services) {
            for (const characteristic of service.characteristics) {
                if (!(0, Characteristic_1.isValidPerms)(characteristic.props.perms)) {
     ...
     */
    permsViolationMessage(violation, disposition) {
        ...
        return `HAP-NodeJS: accessory '${this.displayName}' ${disposition}. Service '${serviceName}' (${violation.service.UUID}), `
            ...
            + `${(0, Characteristic_1.describePerms)(violation.characteristic.props.perms)}`;
    }
    /**
        ...
        for (const accessory of this.bridgedAccessories) {
            ...
            if (violation) {
                if (!accessory.permsQuarantined) {
                    ...
                    accessory.sendCharacteristicWarning(violation.characteristic, "error-message" /* CharacteristicWarningType.ERROR_MESSAGE */, accessory.permsViolationMessage(violation, "is excluded from the accessory database until its permissions are corrected"));
    ...
    async toHAP(connection, contactGetHandlers = true) {
        ...
        if (!this.bridged) {
            accessories.push(...await Promise.all(this.servedBridgedAccessories()
    ...
    async publish(info, allowInsecureRequest) {
        ...
        hksvDebug("[%s] publish: UUID=%s lastKnownUsername=%s newUsername=%s", this.displayName, this.UUID, this.lastKnownUsername, info.username);
