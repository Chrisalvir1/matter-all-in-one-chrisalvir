...
- **Reversión Total Solicitada:**
  - Se restaura íntegramente el código base, componentes de UI y arquitectura de la versión `v1.5.12` (la versión original y funcional con la UI clásica, tarjetas simples y modales limpios).
  - Se mantiene la actualización a **Matterbridge 3.10.8** formalmente en `Dockerfile` y `package.json`.
  - Se eliminan todas las modificaciones experimentales posteriores que alteraron los flujos de emparejamiento y el modal QR.
