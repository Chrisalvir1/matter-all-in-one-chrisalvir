...
export const fanConverter = {
  ...
  presetModeToFanMode(presetMode?: string, isOn: boolean = true): number {
    ...
    if (mode === 'smart' || mode === 'eco') return MatterFanMode.Smart;
    if (mode === 'low' || mode === 'min' || mode === 'silent' || mode === 'sleep') return MatterFanMode.Low;
    if (mode === 'medium' || mode === 'med' || mode === 'mid' || mode === 'normal') return MatterFanMode.Medium;
    if (mode === 'high' || mode === 'max' || mode === 'turbo' || mode === 'boost') return MatterFanMode.High;
   ...
   */
  fanModeToPresetMode(fanMode: number, availableModes: string[] = []): string | undefined {
    ...
    switch (fanMode) {
      ...
      case MatterFanMode.On:
        return findMatching('manual', 'normal', 'standard', 'on') ?? (availableModes.includes('manual') ? 'manual' : undefined);
  ...
  humidifierModeToFanMode(mode?: string, isOn: boolean = true): number {
    ...
    if (m === 'eco' || m === 'smart' || m === 'baby') return MatterFanMode.Smart;
    if (m === 'low' || m === 'sleep' || m === 'intermittent') return MatterFanMode.Low;
    if (m === 'medium' || m === 'normal') return MatterFanMode.Medium;
    if (m === 'high' || m === 'boost' || m === 'continuous') return MatterFanMode.High;
   ...
   */
  fanModeToHumidifierMode(fanMode: number, availableModes: string[] = []): string | undefined {
    const findMatching = (...candidates: string[]) => {
      for (const cand of candidates) {
        const found = availableModes.find((m) => m.toLowerCase() === cand);
        if (found) return found;
    ...
    switch (fanMode) {
      case MatterFanMode.Auto:
        return findMatching('auto') ?? (availableModes.includes('auto') ? 'auto' : undefined);
      case MatterFanMode.Smart:
        return findMatching('eco', 'smart', 'baby', 'auto') ?? (availableModes.length > 0 ? availableModes[0] : undefined);
      case MatterFanMode.Low:
   ...
   */
  humidityToPercentage(humidity: number, minHum: number = 0, maxHum: number = 100): number {
    if (maxHum <= minHum) return Math.max(0, Math.min(100, Math.round(humidity)));
   ...
   */
  percentageToHumidity(percentage: number, minHum: number = 0, maxHum: number = 100): number {
    if (maxHum <= minHum) return Math.max(0, Math.min(100, Math.round(percentage)));
    const humidity = minHum + Math.round((percentage / 100) * (maxHum - minHum));
   ...
   */
  percentageToMode(percentage: number, availableModes: string[] = []): string | undefined {
    if (!availableModes || availableModes.length === 0) return undefined;
    
    // Sort or filter known speed modes
    const speedModes = availableModes.filter((m) => !['auto', 'off'].includes(m.toLowerCase()));
    if (speedModes.length === 0) return availableModes[0];

    const index = Math.min(speedModes.length - 1, Math.floor((percentage / 100) * speedModes.length));
