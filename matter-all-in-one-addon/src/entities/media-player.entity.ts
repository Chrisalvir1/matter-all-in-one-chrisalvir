...
export class MediaPlayerEntity extends BaseEntity {
  declare public endpoint: BasicVideoPlayer;
  ...
  public override async createEndpoint(): Promise<MatterbridgeEndpoint> {
    const name = (this.state.attributes.friendly_name ?? this.entityId).slice(
      0,
      32,
    );
    const serial = this.getMatterSerialNumber();
    this.endpoint = new BasicVideoPlayer(name, serial, {
      onOff: this.state.state !== "off",
    });
  ...
  public override async updateState(
    ...
    const isOn = !["off", "standby", "unavailable", "unknown"].includes(
      state.state,
    );
    const playbackState =
      state.state === "playing"
  ...
  protected override registerCommandHandlers(): void {
    ...
    this.endpoint.addCommandHandler("MediaPlayback.play", async () =>
      this.callService("media_play"),
    );
    this.endpoint.addCommandHandler("MediaPlayback.pause", async () =>
      this.callService("media_pause"),
    );
    this.endpoint.addCommandHandler("MediaPlayback.stop", async () =>
      this.callService("media_stop"),
    );
    this.endpoint.addCommandHandler("MediaPlayback.next", async () =>
      this.callService("media_next_track"),
    );
    this.endpoint.addCommandHandler("MediaPlayback.previous", async () =>
